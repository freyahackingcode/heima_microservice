...
范围查询，一般应用在对==数值类型==做范围过滤的时候。比如做价格范围过滤。
...
- term查询：根据词条精确匹配，一般搜索==keyword类型、数值类型、布尔类型、日期类型==字段
- range查询：根据数值范围查询，可以是==数值、日期==的范围
...
每一个不同的字段，其查询的条件、方式都不一样，必须是多个不同的查询，而要==组合==这些查询，就必须用bool查询了。
